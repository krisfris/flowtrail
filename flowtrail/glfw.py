...
# NOTE:
# Kris Fris modified this source even more.
...
class GLFWimage(Structure):
    ...
    def wrap(self, image):
        ...
        pointer_type = POINTER(ctypes.c_ubyte)
...
joystickfun        = CFUNCTYPE(None, c_int, c_int)
...
#glfwSetWindowIcon              = _glfw.glfwSetWindowIcon
...
def glfwGetJoystickAxes(joy):
    ...
    axes = [c_axes[i] for i in range(count.value)]
    return axes
...
def glfwGetJoystickButtons(joy):
    ...
    buttons = [c_buttons[i] for i in range(count.value)]
    return buttons


def glfwGetJoystickHats(joy):
    count = c_int(0)
    _glfw.glfwGetJoystickHats.restype = POINTER(c_int)
    c_hats = _glfw.glfwGetJoystickHats(joy, byref(count))
    hats = [c_hats[i] for i in range(count.value)]
    return hats


def glfwSetWindowIcon(window, count, images):
    _glfw.glfwSetWindowIcon(window, count, byref(images))
...
def glfwSetJoystickCallback(callback=None):
    global __c_error_callback__
    __c_joystick_callback__ = joystickfun(callback)
    _glfw.glfwSetJoystickCallback(__c_joystick_callback__)
